...
class Gameboard {
  ...
  placeShip(ship, direction, row, column) {
    ...
    if (coordinates.some((e) => this.board[e[0]][e[1]].element !== "sea"))
      return "occupied";
  ...
  isGameOver() {
    if (
      this.board.some((row) =>
        row.some(
          (cell) => cell.element instanceof Ship && !cell.element.isSunk(),
        ),
      )
    )
      return false;
  ...
  isAttackIllegal(row, column) {
    if (row > this.board.length || column > this.board.length) return true;
    if (this.board[row][column].isHit === true) return true;
  }
}
